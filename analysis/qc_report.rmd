...
outlier_qc$pca %>%
  ...
  layout(
    shapes = list(
      vline(pc1_zscore_threshold*sd(outlier_qc$pca$PC1) + mean(outlier_qc$pca$PC1)),
      vline(-pc1_zscore_threshold*sd(outlier_qc$pca$PC1) + mean(outlier_qc$pca$PC1)),
      
      hline(pc2_zscore_threshold*sd(outlier_qc$pca$PC2) + mean(outlier_qc$pca$PC2)),
      hline(-pc2_zscore_threshold*sd(outlier_qc$pca$PC2) + mean(outlier_qc$pca$PC2))
      )
    )
```
...
```{r removed in PCA qc}
if (length(outlier_qc$outliers) > 0) {
  filter(final_md, sample_name %in% outlier_qc$outliers) %>%
...
tar_load(sample_cluster_info)

fviz_gap_stat(sample_cluster_info$gap_stat, maxSE = list(method = "Tibs2001SEmax", SE.factor = 1))

fviz_cluster(
  sample_cluster_info$kmeans_res,
...
t(vsd_exprs)[,c(sex_specific_genes[['symbol']])] %>%
  pheatmap(
    ...
    fontsize = 6,
    show_rownames = FALSE,
    annotation_row = annotation_info[,c("sex", "study_group")],
