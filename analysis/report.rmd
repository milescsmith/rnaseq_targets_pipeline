...
tar_read(module_scores_with_viral)
tar_read(ifn_modules)
```
...
```{r}
tar_read(annotated_modules)
tar_read(annotated_mod_list)
```
...
```{r}
tar_read(module_scores_with_viral)
tar_read(annotated_module_scores)
tar_read(annotated_module_scores_pivot)
tar_read(annotated_module_stats_by_cluster)
tar_read(annotated_module_stats_by_disease)
...
```{r include=FALSE}
tar_read(module_scores_with_viral_by_cluster)
tar_read(module_scores_with_viral_by_cluster_stats)
tar_read(module_scores_with_viral_by_disease)
tar_read(module_scores_with_viral_by_disease_stats)
...
tar_read(wgcna_cluster_rf_cv)
tar_read(wgcna_cluster_test)
...
tar_read(wgcna_cluster_rf_cv_varImp)
...
tar_read(wgcna_disease_class_rf_cv)
tar_read(wgcna_disease_class_test)
...
tar_read(module_cluster_rf_cv)
tar_read(module_cluster_test)
...
tar_read(module_disease_class_rf_cv)
tar_read(module_disease_class_test)
