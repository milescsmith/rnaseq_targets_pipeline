...
```{r}
tar_load(dataset_with_scores)
tar_load(res)
```

```{r study_group_table}
md <- colData(dataset_with_scores) %>% as_tibble(rownames = "sample_name")
if (knitr::is_html_output()){
  md %>%
    tabyl(comparison_grouping_variable) %>%
  ...
  md %>%
    tabyl(comparison_grouping_variable) %>%
    rename(`Disease classification` = comparison_grouping_variable,
           Number = n,
           Percent = percent) %>%
    qflextable() %>%
    set_formatter(
...
for (i in comparison_names){
  ...
  people <-
    filter(
      .data = final_md_tbl,
      {{comparison_sym}} %in% str_split(i, "_vs_", simplify = TRUE)) %>%
    pull(subject)
...
The eigenvalues for the first 100 principal components were used to calculate a
random forest-based adjacentcy matrix, which was in turn used to calculate the gap statistic to determine the optimal *k*-clusters.
...
annotation_info %>%
  mutate(cluster = as_factor(cluster)) %>%
  select({{comparison_sym}}, cluster) %>%
  group_by({{comparison_sym}}) %>%
  mutate(total_disease = n()) %>%
  ungroup() %>%
  group_by({{comparison_sym}}, cluster) %>%
...
p2 <-
  ...
  theme_pubr() +
  geom_mark_ellipse(aes(fill = {{comparison_sym}},
                        color = {{comparison_sym}}),
                    alpha = 0.05
                    ) +
...
annotated_module_group_graphs <-
  ...
  select(-cluster) %>%
  mutate({{comparison_sym}} := as_factor({{comparison_sym}})) %>%
  ggviolin(
