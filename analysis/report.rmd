...
res <- knitr::knit_child(text = unlist(deg_heatmaps), quiet = TRUE)
cat(res, sep = '\n')
```
...
res <- knitr::knit_child(text = unlist(up_deg_enrichment), quiet = TRUE)
cat(res, sep = '\n')
```
...
res <- knitr::knit_child(text = unlist(down_deg_enrichment), quiet = TRUE)
cat(res, sep = '\n')
```
...
```{r Volcano_plot_of_gene_expression, echo=FALSE, fig.height=9, fig.width=9}
volcanoes <- purrr::map2(
  .x = res,
  .y = names(res),
  .f = \(x, y)
