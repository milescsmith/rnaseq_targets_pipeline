...
if (knitr::is_html_output()){
  ...
  md %>%
    tabyl(study_group) %>%
    rename(`Disease classification` = study_group,
           Number = n,
           Percent = percent) %>%
    qflextable() %>%
    set_formatter(
...
for (i in comparison_names){
  ...
  people <-
    filter(
      .data = final_md_tbl,
      study_group %in% str_split(i, "_vs_", simplify = TRUE)) %>%
    pull(subject)
...
annotation_info %>%
  mutate(cluster = as_factor(cluster)) %>%
  select(study_group, cluster) %>%
  group_by(study_group) %>%
  mutate(total_disease = n()) %>%
  ungroup() %>%
  group_by(study_group, cluster) %>%
...
```{r PCA_by_study_groupification, fig.width=12, fig.height=6}
p1 <-
  ggplot(
    data = pca_results,
    aes(
...
<!--   ) %>% -->
<!--   add_markers( -->
<!--     text = str_glue("Sample: {pca_results$sample_name}<br>Classification: {pca_results$study_group}<br>Ethnicity {pca_results$ethnicity}<br>Cluster: {pca_results$cluster}<br>Sex: {pca_results$sex}<br>Plate: {pca_results$run_id}"), -->
<!--     color = ~cluster, -->
<!--     size = 2.5, -->
<!--     alpha = 0.9, -->
<!--     hovertemplate = "%{text}", -->
...
```{r UMAP_by_study_groupification, fig.width=12, fig.height=7}
p1 <-
  ggplot(
    data = umap_results,
    aes(
...
<!--   ) %>% -->
<!--   add_markers( -->
<!--     text = str_glue("Sample: {umap_results$sample_name}<br>Classification: {umap_results$study_group}<br>Ethnicity {umap_results$ethnicity}<br>Cluster: {umap_results$cluster}<br>Sex: {umap_results$sex}<br>Plate: {umap_results$run_id}"), -->
<!--     color = ~cluster, -->
<!--     size = 2.5, -->
<!--     alpha = 0.9, -->
<!--     hovertemplate = "%{text}", -->
...
module_scores_with_viral %>%
  ...
  pheatmap(
    ...
    annotation_col = arrange(annotation_info[,"study_group",drop=FALSE], study_group),
    breaks = zscore_range,
    cluster_rows = FALSE,
    cluster_cols = FALSE,
    color = viridis(n = length(zscore_range)-1, option = "D"),
...
module_scores_with_viral %>%
  ...
  pheatmap(
    ...
    annotation_col = annotation_info[,c("cluster", "study_group"),drop=FALSE],
    breaks = zscore_range,
    cluster_rows = FALSE,
    cluster_cols = TRUE,
    color = viridis(n = length(zscore_range)-1, option = "D"),
...
annotated_module_study_group_graphs <-
  annotated_module_scores_pivot %>%
  select(-cluster) %>%
  mutate(study_group = as_factor(study_group)) %>%
  ggviolin(
    x = "study_group",
    y = "score",
    fill = "study_group",
...
tar_load(wgcna_study_group_rf_cv)
tar_load(wgcna_study_group_test)

caret::confusionMatrix(
  data = predict(wgcna_study_group_rf_cv, wgcna_study_group_test),
  reference = as_factor(wgcna_study_group_test$study_group)
)
```
...
tar_read(wgcna_study_group_rf_cv_varImp)
```
...
tar_load(module_study_group_rf_cv)
tar_load(module_study_group_test)

caret::confusionMatrix(
  data = predict(module_study_group_rf_cv, module_study_group_test),
  reference = as_factor(module_study_group_test$study_group)
)
```
...
tar_read(module_study_group_rf_cv_varImp)
```
