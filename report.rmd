...
t(vsd_exprs)[,unique(c(top_up[1:50], top_down))] %>% 
    pheatmap(
      scale = 'column',
      fontsize = 9,
      show_rownames = FALSE,
...
plot_grid(
  pca_results %>%
    ...
    scale_color_paletteer_d(package = "ggthemes", palette = "calc") +
    scale_fill_paletteer_d(package = "ggthemes", palette = "calc") +
    labs(color = "project") +
    theme(legend.position = "none"),
  pca_results %>%
    ggplot(aes(x = PC1,
...
```{r, fig.width=12, fig.height=6}
plot_grid(pca_plot2 + 
            scale_color_paletteer_d(package = "ggsci", palette = "default_igv") +
            scale_fill_paletteer_d(package = "ggsci", palette = "default_igv") +
            theme(legend.position = "none"),
          pca_plot2 +
...
zscore_range <- c(-range_bound:range_bound)
ifn_scores_with_viral %>%
  #select(sample_name, one_of(viral_transcripts)) %>%
  column_to_rownames("sample_name") %>%
  pheatmap(
    scale="column",
    fontsize = 9,
...
loadd(wgcna_scores)
wgcna_scores %>%
  select(disease_class, starts_with("ME")) %>%
...
caret::confusionMatrix(
  ...
  reference = wgcna_cluster_test$cluster
)
```
