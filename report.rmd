...
### PCA, by disease group
```{r, fig.width=12, fig.height=6}
plot_grid(
  ggplot(data = pca_results,
         mapping = aes(x = PC1,
                       y = PC2,
                       fill = disease_grouping)) +
    geom_point(shape = 21,
               colour = "black",
               size = 2,
               stroke = 0.25,
  ...
  ggplot(data = pca_results,
    ...
    labs(color = "Group",
         fill = "none") +
    geom_mark_ellipse(aes(fill=disease_grouping,
                          color=disease_grouping),
                      alpha = 0.05)
)
```
...
module_scores_with_viral %>%
  select(disease_grouping,
         starts_with("ME")) %>%
  pivot_longer(-disease_grouping,
               names_to = "module",
               values_to = "score") %>%
  mutate(disease_grouping = as_factor(disease_grouping)) %>%
  ggplot(aes(x = disease_grouping,
             y = score,
             fill = disease_grouping)) +
  geom_violin() +
  geom_jitter(size = 0.25, alpha = 0.25) +
...
loadd(MEplotting)
map(1:ceiling(length(unique(MEplotting$module))/6),
    function(i){
