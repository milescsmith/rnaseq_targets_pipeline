analysis_plan = drake_plan(
  ...
  md =
    ...
    mutate(
      disease_class = recode(disease_class, "NL" = "Control"),
  ...
  # sledai = read_xlsx(path = sledai_bpx_file,
  #                    sheet = "SLEDAI",
  #                    col_types = c("text",
  #                                  "text",
  #                                  "date",
  ...
  banchereau_module_scores =
    tirosh_score_modules(expr_obj = vsd_exprs, module_list = module_list) %>%
    as_tibble(rownames = "sample_name"),

  ldg_module_scores = tirosh_score_modules(expr_obj = vsd_exprs, module_list = ldg_modules) %>%
    as_tibble(rownames = "sample_name"),

  metasig_scores = tirosh_score_modules(vsd_exprs, metasignature_module) %>%
    as_tibble(rownames = "sample_name"),

  module_scores = inner_join(banchereau_module_scores, ldg_module_scores) %>%
  ...
  dds_with_scores = target({
    colData(dds_processed) <-
      left_join(
        as_tibble(colData(dds_processed), rownames = "sample"),
        as_tibble(module_scores, rownames = "sample")
      ) %>%
  ...
  cytokine_values = read_csv(file = clinical_file) %>%
    filter(cle_type != "Control") %>%
    set_names(nm = recode(names(.), !!!cytokine_names)) %>%
    select(sample_name, one_of(cytokine_names)),

  cytokine_pivot =
    cytokine_values %>%
    pivot_longer(-sample_name, names_to = "cytokine"),

  module_scores_for_correlation = module_scores_with_viral %>%
    select(sample_name, `M4.10`,	`M4.11`,	`M4.1`,	`M4.15`,	`M3.6`,	`M8.46`,	`M4.14`,	`M5.15`,
           `LDG1.1`,	`LDG2.1`,	`M8.83`,	`M3.2`,	`M4.2`,	`M4.6`,	`M4.13`,	`M5.1`,	`M5.7`,
           `M7.1`,	`M1.2`,	`M3.4`,	`M5.12`,	`M6.6`,	`M2.2`,	`M3.3`,	`M3.5`,	`M4.7`,	`M6.11`,
           `M6.16`, `M9.42`,	`M6.13`,	`M5.6`,	`M5.10`,	`M6.2`,	`M6.12`,	`M4.3`,	`M4.5`,
           `M5.9`,	`M1.1`,	`M2.3`,`M3.1`,	`M6.18`) %>%
    set_names(nm = c("sample_name", "M4.10_Bcell",	"M4.11_PlasmaCells",	"M4.1_Tcells",	"M4.15_Tcells",
                     "M3.6_Cytotoxic_NK",	"M8.46_Cytotoxic_NK",	"M4.14_Monocytes",	"M5.15_Neutrophils",
                     "LDG1.1",	"LDG2.1",	"M8.83_ImmuneResponses",	"M3.2_Inflammation",	"M4.2_Inflammation",
                     "M4.6_Inflammation",	"M4.13_Inflammation",	"M5.1_Inflammation",	"M5.7_Inflammation",
                     "M7.1_Inflammation",	"M1.2_Interferon",	"M3.4_Interferon",	"M5.12_Interferon",
                     "M6.6_Apoptosis_Survival",	"M2.2_CellCycle",	"M3.3_CellCycle",	"M3.5_CellCycle",
                     "M4.7_CellCycle",	"M6.11_CellCycle",	"M6.16_CellCycle",	"M9.42_CellCycle",
                     "M6.13_CellDeath",	"M5.6_Mitochondrial",	"M5.10_Mitochondrial",	"M6.2_Mitochondrial",
                     "M6.12_Mitochondrial",	"M4.3_ProteinSynthesis",	"M4.5_ProteinSynthesis",
                     "M5.9_ProteinSynthesis",	"M1.1_Platelets",	"M2.3_Erythrocytes",	"M3.1_Erythrocytes",
                     "M6.18_Erythrocytes")
    ),
