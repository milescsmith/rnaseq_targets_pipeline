...
```{r 25_top_down_DE_tbl, echo = FALSE}
...
<!-- #### sort by cell_type ####
sample_cell_type_order_info <-
  annotation_info %>%
  as_tibble(rownames = "subject") %>%
  arrange(cell_type)

sample_cell_type_order <-
  sample_cell_type_order_info %>%
  pull(subject)
  
sample_cell_type_order_count <-
  sample_cell_type_order_info %>%
...
<!-- ### Sorted by cell type
```{r Expression_of_all_top_DE_genes_cell_type_sort, fig.width=12, fig.height=9}
sample_cell_type_order <-
  annotation_info %>%
  as_tibble(rownames = "subject") %>%
  arrange(cell_type) %>%
...
range_bound = 4
zscore_range <- c(-range_bound:range_bound)
t(vsd_exprs)[sample_cell_type_order,colnames(deg_means)] %>% 
    pheatmap(
      scale = 'column',
      fontsize = 9,
      fontsize_col = 6,
      ...
      cluster_rows = FALSE,
      color = viridis(n = length(zscore_range)-1, option = "D"),
      gaps_row = sample_cell_type_order_count,
      border_color = NA
      )
``` -->
...
<!-- ### Sorted by cell_type
```{r Expression_of_ISGs_cell_type_sort, fig.width=12, fig.height=9}
range_bound = 4
zscore_range <- c(-range_bound:range_bound)
t(vsd_exprs)[sample_cell_type_order,ISGs] %>%
    pheatmap(
      scale = 'column',
      fontsize = 8,
      show_rownames = FALSE,
...
```{r module_ISGs_cell_type_sort, fig.width=12, fig.height=9}
range_bound = 4
zscore_range <- c(-range_bound:range_bound)
t(vsd_exprs)[sample_cell_type_order,(arrange(as_tibble(module_ISGs, rownames = "gene"), module)$gene)] %>%
  pheatmap(
    scale = 'column',
    fontsize = 6,
    show_rownames = FALSE,
...
```{r PCA_by_cell_type, fig.width=12, fig.height=6}
p1 <-
  ggplot(
    data = pca_results,
    aes(
...
p2 <-
  ...
  scale_color_brewer(palette = "Set1") +
  theme_pubr() +
  geom_mark_ellipse(aes(fill = cell_type,
                        color = cell_type),
                    alpha = 0.05
                    ) +
  theme(legend.position = "bottom")
...
<!-- ### UMAP by cell type
```{r UMAP_by_cell_type, fig.width=12, fig.height=7}
p1 <-
  ggplot(
    data = umap_results,
    aes(
...
<!-- ### Sorted by cell_type
```{r SLE_Interferon_module_scores_cell_type_sort, fig.width=3, fig.height=6}
range_bound = 4
zscore_range <- c(-range_bound:range_bound)
module_scores_with_viral %>%
  select(sample_name, one_of(ifn_modules)) %>%
  arrange(match(sample_name, sample_cell_type_order)) %>%
  column_to_rownames("sample_name") %>%
  pheatmap(
    scale="column",
    fontsize = 9,
...
<!-- ### Sorted by cell_type
```{r SLE_Inflammatory_module_scores_cell_type_sort, fig.width=4, fig.height=6}
range_bound = 4
zscore_range <- c(-range_bound:range_bound)
module_scores_with_viral %>%
  select(sample_name, one_of(inflame_modules)) %>%
...
<!-- ### Sorted by cell_type
```{r ldg_module_cell_type_sort, fig.width=3.25, fig.height=6}
module_scores_with_viral %>%
    select(sample_name,
         one_of(names(ldg_modules))) %>%
  arrange(match(sample_name, sample_cell_type_order)) %>%
...
<!-- ### Sorted by cell_type
```{r annotated_modulecell_type_sort, fig.width=12, fig.height=9}
module_scores_with_viral %>%
  arrange(cell_type) %>%
  select(
    sample_name,
  ...
  pheatmap(
    ...
    annotation_colors = group_pal,
    annotation_row = column_to_rownames(mutate(annotated_modules, module = recode(module, !!!annotated_mod_list)), var = "module"),
    annotation_col = arrange(annotation_info[,"cell_type",drop=FALSE], cell_type),
    breaks = zscore_range,
    cluster_rows = FALSE,
    cluster_cols = FALSE,
    color = viridis(n = length(zscore_range)-1, option = "D"),
    angle = 315,
    gaps_col = sample_cell_type_order_count,
    gaps_row = module_gaps,
    border_color = NA)
``` -->
...
<!-- ### Sorted by cell_type
```{r all_module_cell_type_sort, fig.width=12, fig.height=9}
module_scores_with_viral %>%
  select(
    sample_name,
    matches("^M[[:digit:]]+")
...
<!-- ### By cell_type
```{r Banchereau_modules_by_cell_type, echo=FALSE, fig.height=8, fig.width=12, warning=FALSE}

annotated_module_cell_type_graphs <-
  annotated_module_scores_pivot %>%
  ...
  scale_fill_paletteer_d("ggsci::uniform_startrek") +
  stat_pvalue_manual(
    data = annotated_module_stats_by_cell_type,
    label = "p.adj.signif",
    hide.ns = TRUE,
    bracket.size = 0.1,
    tip.length = 0.01,
...
<!-- ### By cell_type
```{r wgcna_modules_by_cell_type, echo=FALSE, fig.height=8, fig.width=12}
number_of_modules <-
  module_scores_with_viral %>%
  select(matches("^ME"))%>%
  ncol()
...
module_cell_type_graphs <-
  ...
  scale_fill_paletteer_d("ggsci::uniform_startrek") +
  stat_pvalue_manual(
    data = module_scores_with_viral_by_cell_type_stats,
    label = "p.adj.signif",
    hide.ns = TRUE,
    bracket.size = 0.1,
    tip.length = 0.01,
...
<!-- 
### Sorted by cell_type
```{r WGCNA_eigenvalues_cell_type_sort, fig.width=12, fig.height=9}
module_scores_with_viral %>%
  select(
    sample_name,
    matches("^ME")
